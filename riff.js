...
var riffGlobal = new (function()
{
	...
	//h storage의 데이터 저장 유효 기간( cookies 일때만 적용 ) ???? 
	this.storageDate = null;

	// get data from cache( if true )
	//h RSS Component를 이용하여 화면을 구성할 때 Storage에 화면 정보에 대해 저장된 값이 있다면, 최초 1회( widget이 아래의 tray에 내려갔다가 다시 올라올 때) 만 Cache의 데이터를 가져온다.
	//h 2회째라면, RSS Component가 Cache가 아닌 통신을 이용해 서버의 데이터를 가져온다.
	this.useRssCache = false;
...
riff.fn = riff.prototype = {
	...
	//h 질의된 DOM Element의 자식 DOM Element 들을 선택한다.
	...
	//h _idx가 범위를 벗어난 경우, 빈 Framework Object를 반환한다. 
	...
	//h 인자값:
	//h		_data : selector( 문자열 ) 혹은 Framework 객체.
	//h			selector ( 문자열 ) 이라면, 해당 문자열로 질의된 DOM Element 들을 추가한다.
	//h			framework 객체라면, framework객체가 담고 있는 DOM Element 들을 추가한다.
	//h 반환값:
	//h 	riff 객체.	
	// add
	add : function( _data )
	...
	//h Framework 객체가 가진 DOM Element 를 반환한다.
	...
	//h Framework 객체가 가진 component 를 반환한다.
	//h _idx가 범위를 넘을 경우, null을 반환한다.
	//h 인자값:
	//h		_idx : 반환 받을 DOM Element의 순서( 숫자 ).
	//h 반환값:
	//h 	값이 있을 경우 : Framework Component.
	//h 	값이 없을 경우 : null.
	// component
	...
	//h 	_name, _value : Framework 객체를 반환.
	// css
	...
	css : function ( _name, _value )
	{
		...
		{
			//h 만일 사용자가 css value로 문자열을 입력하지 않았으면, 그냥 framework 객체를 반환하고 끝내자.
	...
	//h 반환값:
	//h		인자값이 없을 때 : 선택된 DOM Element 중 첫번째 DOM Element의 innerHTML String를 반환.
	//h		인자값이 _htmlCode : Framework 객체 반환
	// html
	...
	html : function( _htmlCode )
	{
		if ( arguments.length == 0 ) {
			...
			//h 만일 사용자가 _htmlCode로 문자열을 입력하지 않았으면, 그냥 framework 객체를 반환하고 끝내자.
	...
	text : function ( _text )
	{
		if( _text )
		{
			//h 만일 사용자가 _text로 문자열 혹은 숫자를 입력하지 않았으면, 그냥 framework 객체를 반환하고 끝내자.
	...
	//h 선택된 DOM Element 각각이 htmlCode에 해당되는 DOM Element의 자식 노드가 된다.
	//h 인자값:
	...
	{
		//h 만일 사용자가 _content 문자열을 입력하지 않았으면, 그냥 framework 객체를 반환하고 끝내자.
		if( typeof( _content ) != 'string') return this;
	...
	//h 선택된 DOM Element 모두가 htmlCode에 해당되는 DOM Element의 자식 노드가 된다.
	//h 인자값:
	//h		_content : 선택된 DOM Element 를 감쌀 html code.
	//h 반환값:
	//h		Framework Object
	...
	//h 선택된 DOM Element들의 다음에 htmlCode에 해당하는 DOM Element를 삽입한다.
	//h 인자값:
	...
	//h 선택된 DOM Element들의 이전에 htmlCode에 해당하는 DOM Element를 삽입한다.
	//h 인자값:
	...
	//h _func : animation이 실행되고 난 후 실행되는 함수.
	//h _preFunc : animation 실행되기 전에 실행되는함수.
        ...
        if(!this.buffer("riffAnimateInitFlag")) 
        {
			...
			{(
		        function() 
		        {
					...
					{
						//h riffAnimateQueue 로 예약된 animation queue 를 가져온다 -> 화면에서 동시에 2개 이상의 객체를 animation하려면, 에러난다. ckbcorp
	...
	//h    @ _existEventName : 이벤트 이름(문자열). ex: "onfocus"
	//h    RETURN : this.
	...
	//h 이벤트 추가 공통 함수. 전역변수설정에 있는 "touch"( widget ) / "mouse"( PC ) 값으로, 등록해야 할 이벤트를 판단. 
	...
	//h component의 setOKFunc() 함수 실행
	// setOKFunc
	//	component function
	setOKFunc : function( _value )
	{
		var args = arguments;
		...
		{
			var c = riff(this);
			if(c.component().setOKFunc)
				c.component().setOKFunc.apply(this, args);
		});
	},

	//h component의 select() 함수 실행
	//h 해당 컴포넌트 데이터
	//h 첫번째 것만 반환
	// select
	//	component function
...
//h 새 링크로 이동하는 함수. phone이면 widget함수 이용. PC라면 새창띄운다.
...
//h emulator 이면 true, widget 이면 false, pc 면 false
...
//h #id 혹은 id 로 접근. 
//h 글로벌 변수엔 # 붙인것까지 저장. 
//h _page : 이동할 DOM Element의 ID. Framework Components로 구성.
...
riff.back = function( _option )
{
	if( typeof _option == "object" || typeof _option =="undefined" ) {
		return riff.scene.apply( _option );
	}
};
...
//h 페이지로 이동. 
//h #id 혹은 id 로 접근
//h _scene : 이동할 DOM Element의 ID. Framework Components로 구성.
//h _option : "transitionEffect", "transitionMotion", "transitionSecond", "transitionFunc"
//h		설정 가능
...
riff.storage = function ( _key, _value, _date )
{
	if( !_key ) {
		return null;
	}
	...
	if(arguments.length == 2 || arguments.length == 3)
	{
		if( riff.isWidget() ) 
			...
			riff.storage.localStorage( _key, _value, _date );
		}		
		return true;
	}
...
//	@_date : javascript Date().getTime() format( unix-style integer time value ).  time limit to store data. 
//
riff.storage.localStorage = function ( _key, _value, _date )
{
	if ( !_key )
		return null;
	...
	if( arguments.length == 1) {
		...
		if ( t && i > 0 ){
			//h 날짜정보만 꺼내온다
			var d = t.substring( 0, i );
			//h 원래 문자열을 꺼내온다
			var s = t.substring( i + 2 );

			if ( d == 0 || d == "0" )	{
				return s;	
			} else if ( window.parseInt(d) > ( new Date().getTime() ) )	{
				//h 유통기한이 현재 시간 이후라면
				return s;
			}

			//h 기한이 지난 데이터로 판단, 삭제.
			storageObj.removeListItem( _key );
			return false;
			
		} else {
			//h 배열 형태가 아니므로( 아마도 string, number, "", null 중 하나 ) 그대로 반환
			return storageObj.getItem( _key );
		}
	}else if( arguments.length == 2 ) { 
		//h 데이터는 무조건 "시간;;저장할 문자열" 의 형태로 저장한다.
		return storageObj.setItem( _key, "0;;" + _value );
	} else {
		var d = window.parseInt( _date );

		if ( isNaN( d ) ) {	
			//h 들어온 _date 값이 Date().getTime() 값이 아니면
			return false;
		}
		// key, value, date
		return storageObj.setItem( _key, d + ";;" + _value );
	}
	return true;
};
...
riff.load = function( _func ) 
{
	...
	//h _func 가 없으면 함수 꺼짐
	//h 실제로는, onload 이벤트를 이용해야 하는데 객체가 생성되지 않으면 onload를 생성할 수 없으므로,
	//h window 와 body 객체 이외에는 onload 를 이용할 수 없다. 
	// load
	//   bind a load function
...
//h rss데이터 수신시간 보여주기
riff.feedReceiveTime.on = function ( )
{
	riff(".feedReceiveTime").show();
...
//h rss데이터 수신시간 감추기
riff.feedReceiveTime.off = function ( )
{
	riff(".feedReceiveTime").hide();
