...
riff.dataReceiveTime = function ( _showTime, _showText )
{
	...
	var idleShowTime = _showTime;
	var idleShowText = _showText;
	if ( arguments.length == 0 )
	{
		tBuf.hide();
		tBuf.html("");
	} else if( arguments.length == 1 ) {
		tBuf.html( _showTime + "<span class='rf-style-recieveinfo'>" + tBuf.buffer("feedDataReceiveText")+"</span>" );
		tBuf.show();
		tBuf.buffer("feedDataReceiveTime", _showTime );
		idleShowText = tBuf.buffer("feedDataReceiveText");
	} else if( arguments.length == 2 ) {
		tBuf.html( _showTime + "<span class='rf-style-recieveinfo'>" + _showText+"</span>" );
		tBuf.show();
		tBuf.buffer("feedDataReceiveTime", _showTime );
		tBuf.buffer("feedDataReceiveText", _showText );
	};
