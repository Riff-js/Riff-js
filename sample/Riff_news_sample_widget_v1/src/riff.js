	...
	if( typeof _scene == "string" ) {
		...
		if( scene == "#idle" ){
		    ...
		    if( beforeIdle ) {
				...
				var tabID = riff("#idle").buffer("ComponentDataFEEDComponentID");

				// feedTab Index
				var tabMenuIndex = riff(tabID).buffer("ComponentTabMenuIndex");
				// change feedTab Index
				riff("#idle").buffer("ComponentDataFEEDKey", tabMenuIndex);

				// title
				var feedComponent = riff("#idle").buffer("ComponentDataFEEDComponent");
				...
				var reduceArticleText = riff.ellipsisString( idleSet[0].txtHead, 60 );
		...
		if( riffSceneTabMenuList.buffer("ComponentListComponentDataFEEDFlag") ) {
			// If a Tab Component using FEED exists, the FEED value of the TAB Component is returned
			riff.dataReceiveTime( riffSceneTabMenuList.buffer("ComponentListComponentDataFEEDTime") );
		} else if ( ( riffSceneTabMenu.size() == 0 ) &&  riffSceneList.buffer("ComponentListComponentDataFEEDFlag") ) {
			// If a List Component using FEED exists, the FEED value of the List Component is returned
			riff.dataReceiveTime( riffSceneList.buffer("ComponentListComponentDataFEEDTime") );
		} else {
...
riff.dataReceiveTime = function ( _showTime, _showText )
{
	if ( arguments.length == 0 )
	{
		...
		riff(".rf-component-datareceivetime").buffer("feedDataReceiveTime", "" );
		riff(".rf-component-datareceivetime").html("");
	} else if( arguments.length == 1 ) {
		riff(".rf-component-datareceivetime").show();
		riff(".rf-component-datareceivetime").html( _showTime );
		riff(".rf-component-datareceivetime").buffer("feedDataReceiveTime", _showTime );
	} else if( arguments.length == 2 ) {
		if( arguments[0] == null ) {
			riff(".rf-component-datareceivetime").html( riff(".rf-component-datareceivetime").buffer("feedDataReceiveTime") + " " + _showText );
			riff(".rf-component-datareceivetime").show();
		} else {
			riff(".rf-component-datareceivetime").buffer("feedDataReceiveTime", _showTime );
			riff(".rf-component-datareceivetime").html( _showTime + " " + _showText );
			riff(".rf-component-datareceivetime").show();
		}
	}
	return true;
...
{
	riff.dataReceiveTime( null, _showText );
	return true;
}
...
riff.frameworkTabMenu = function ()
{
	...
	for( var k in scenePartArray )
	{
		dataSet[k] = ["<div class='rf-component-list' id='ComponentTabMenuFEEDList"+k+"'><div class='rf-style-nodata' style='display:none;'>No data to display</div></div>",
			...
			function ()
			{
				riff.dataReceiveTime( riff.currentTime() );
			}
		];
			var dataSetFEEDCur = {};
