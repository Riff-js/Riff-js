...
riff.component.list.prototype.createItem = function(_obj,_type){	
	if (!_type) _type = "node";		
	...
	if (typeof _obj == "string"){
		...
		//d _timeString = "2011-03-20T14:45:00Z"
		...
		if(_obj.tit){
			...
			riff.manipulation.appendStr([tEle],_obj.tit);
			riff.manipulation.appendNode([tlistItem],tEle);
		};
		...
		if(_obj.updated){
			tCreate(riff.widget.currentTimeParsing(_obj.updated),"rf-component-item-itemInfo")
		};
		if(_obj.credit){
			tCreate(_obj.credit,"rf-component-item-itemInfo")
		};
		
		if(_obj.link){
			var tUrl = _obj.link;
			riff.event.tap([tlistItem],function(){
			riff.widget.openURL( tUrl );
		})
	}
...
riff.component.list.prototype.createOrdering = function(){
...
//		for(var i=0,iLen = this.tempContent.length;i<iLen;i++){
//			this.tempContent[i].checked = false;
//		}
...
//		for(var i=0,iLen = this.tempContent.length;i<iLen;i++){
//			this.tempContent[i].checked = true;
//		}		
...
//		if(this.tempContent[i].checked) tCnt++;
//	}
	if(tCnt == tLen){
		this.allChecked = true;
